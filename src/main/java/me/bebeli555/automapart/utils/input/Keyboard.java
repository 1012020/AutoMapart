...
import me.bebeli555.automapart.utils.input.Mouse.IntButton;
...
public class Keyboard {
    ...
    private void onKey(KeyInputEvent e) {
        ...
        if (e.getAction() == 1) {
            if (!down.contains(new IntButton(e.getKey()))) {
                down.add(new IntButton(e.getKey()));
            }
        } else if (e.getAction() == 0) {
            down.remove(new IntButton(e.getKey()));
        }
    }
    ...
    public static boolean isKeyDown(int key) {
        return down.contains(new IntButton(key));
    }

    public static char getEventCharacter() {
        return (char)eventKey;
    }
