...
public class AutoMapart extends Mod {
    ...
    public static Setting instantChestExtraction = new Setting(Mode.BOOLEAN, "InstantChestExtraction", true, "If true, extracts carpets from chests instantly (zero delay); if false, uses the configured delay");
    ...
    public void loop() {
        ...
        if (finished) {
            if (resetMachine) {
                ...
                // Inventory drop: drop all carpets at once (no per-slot delay)
                if (mc.player.playerScreenHandler != null) {
                    List<Integer> carpetSlots = new ArrayList<>();
                    for (Slot slot : mc.player.playerScreenHandler.slots) {
                        if (slot.getStack() != null && Block.getBlockFromItem(slot.getStack().getItem()) instanceof CarpetBlock) {
                            carpetSlots.add(slot.id);
                        }
                    }
                    if (!carpetSlots.isEmpty()) {
                        if (chatDebug.bool()) sendMessage("Dropping carpets from inventory");
                        run(() -> RotationUtils.rotateTo(mc.player.getBlockPos().add(2, 1, 0)));
                        for (Integer slotId : carpetSlots) {
                            run(() -> InventoryUtils.throwAway(slotId));
                        }
                        return;
                    }
                }
                ...
                if (hasLockedMap) {
                    if (mc.currentScreen instanceof GenericContainerScreen) {
                        ...
                        // Chest extraction: choose method based on our boolean setting
                        if (instantChestExtraction.bool()) {
                            // Instant extraction: all quick moves happen immediately
                            ...
                            // Normal extraction: use delay between each quick move
                            for (Slot slot : mc.player.currentScreenHandler.slots) {
                                if (slot.id <= 53 && slot.getStack().getItem() == Item.fromBlock(blockToPlace)) {
                                    if (InventoryUtils.getFreeSpace() <= 0) break;
                                    run(() -> InventoryUtils.quickMove(slot.id));
                    ...
                    sleep(1000);
                    sleepUntil(() -> !BaritoneUtils.isPathing(), 60000);
                } else if ((InventoryUtils.getAmountOfItem(Items.MAP) == 0 && InventoryUtils.getAmountOfItem(Items.FILLED_MAP) == 0)
        ...
        if (InventoryUtils.getAmountOfItem(Item.fromBlock(blockToPlace)) == 0) {
            ...
            if (!validChests.isEmpty()) {
                ...
                if (BlockUtils.distance(mc.player.getBlockPos(), nearestChest) <= 4) {
                    if (mc.currentScreen instanceof GenericContainerScreen) {
                        if (chatDebug.bool()) sendMessage("Taking carpets from chest");
                        // Use the instant mode based on the boolean setting
                        if (instantChestExtraction.bool()) {
                            for (Slot slot : mc.player.currentScreenHandler.slots) {
                                if (slot.id <= 53 && slot.getStack().getItem() == Item.fromBlock(blockToPlace)) {
                                    if (InventoryUtils.getFreeSpace() <= 0) break;
